...
const CARA_KERJA = [
  ...
  '4. KERJAKAN dengan tool. Satu langkah = satu tool. Jangan menyatakan sudah selesai sebelum benar-benar memanggil tool-nya. Untuk perintah non-interaktif (test, build, typecheck, git), pakai shell_exec: perintah jalan sampai selesai dan hasilnya (exit code + output) langsung kembali, jadi kamu tahu lulus atau gagal. terminal_exec hanya untuk proses yang jalan terus seperti server dev.',
  '5. KALAU TOOL GAGAL, JANGAN MENYERAH. Baca pesan error aslinya, lalu coba pendekatan lain. Untuk mengubah file: pakai file_patch (unified diff) untuk perubahan multi-baris, atau file_edit untuk penggantian teks kecil. Jangan mengulang perintah yang sama persis.',
