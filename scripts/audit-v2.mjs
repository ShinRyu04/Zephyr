// Audit FULL v2: buka settings DULU dengan klik activity 'settings' + pastikan
// panel terbuka (cek set-nav-general muncul), BARU loop semua tab & kontrol.
...
async function evalJs(expr, awaitPromise = false) {
  ...
  if (r.result?.exceptionDetails) return { err: r.result.exceptionDetails.text + ' ' + String(r.result.exceptionDetails.exception?.description || '').slice(0, 120) };
  return r.result?.result?.value;
}
const sleep = (ms) => new Promise((r) => setTimeout(r, ms));

// Buka settings + VERIFIKASI terbuka
...
await sleep(1200);
const terbuka = await evalJs('document.querySelectorAll("[data-testid^=set-nav-]").length');
console.log('nav settings terdeteksi:', terbuka);
if (!terbuka) {
  // klik lagi (mungkin toggle menutup)
  await evalJs(`(() => { const b = [...document.querySelectorAll('[data-activity]')].find(e => e.getAttribute('data-activity') === 'settings'); if (b) b.click(); return 1; })()`);
  await sleep(1200);
  console.log('nav setelah klik 2x:', await evalJs('document.querySelectorAll("[data-testid^=set-nav-]").length'));
}
...
for (const l of laporan) console.log(`[${l.tab.replace('set-nav-', '')}] ${l.id}: ${l.hasil} (${l.detail})`);
